...
class FasttextClassifier(BaseEstimator, ClassifierMixin):
    ...
    def fit(self, x, y):
        ...
        self.classifier = ft.supervised(temp_file, self.output, dim=self.dim, lr=self.lr,
                                        lr_update_rate=self.lr_update_rate,
                                        epoch=self.epoch, min_count=self.min_count, word_ngrams=self.word_ngrams,
                                        bucket=self.bucket, thread=self.thread, silent=self.silent,
                                        label_prefix=self.lpr)
        return (None)
